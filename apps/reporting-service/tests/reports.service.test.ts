...
describe('ReportsService', () => {
  ...
  it('should return site summary with provided crawlJobId', async () => {
    vi.mocked(SiteIssueSummaryRepository.getSiteSummary).mockResolvedValue({
      pages_crawled: 10,
    } as any);

    const result = await ReportsService.getSiteSummary(SITE_ID, JOB_ID);

    expect(SiteIssueSummaryRepository.getSiteSummary).toHaveBeenCalledWith(
      SITE_ID,
      JOB_ID,
      undefined,
      undefined
  ...
  it('should resolve crawl job by date', async () => {
    vi.mocked(CrawlJobRepository.findByDate).mockResolvedValue({
      id: JOB_ID,
    } as any);
    ...
    expect(IssueAnalyticsRepository.getSeverityBreakdown).toHaveBeenCalledWith(
      SITE_ID,
      JOB_ID,
      expect.any(Date),
      expect.any(Date)
  ...
  it('should fallback to latest crawl job', async () => {
    vi.mocked(CrawlJobRepository.findLatest).mockResolvedValue({
      id: JOB_ID,
    } as any);

    vi.mocked(PageIssueSummaryRepository.getTopPages).mockResolvedValue([]);

    const result = await ReportsService.getTopPages(SITE_ID);

    expect(CrawlJobRepository.findLatest).toHaveBeenCalled();

    expect(PageIssueSummaryRepository.getTopPages).toHaveBeenCalledWith(
      SITE_ID,
      JOB_ID,
      undefined,
      undefined
  ...
  it('should return issues with pagination', async () => {
    ...
    vi.mocked(IssueAnalyticsRepository.getIssues).mockResolvedValue({
      rows: [],
      count: 0,
    } as any);

    const result = await ReportsService.getIssues(SITE_ID, 'critical', 10, 2);

    expect(IssueAnalyticsRepository.getIssues).toHaveBeenCalledWith(
      SITE_ID,
      'critical',
      10,
      10,
